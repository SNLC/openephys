...
for i=1:length(lightON_all)
    ...
    for ch = 1:nchs
        chFilt = filtfilt(bf, af, rawmat(ch,:)); % low-pass filtered signal (for constructing artifact template)
        chFilt2 = filtfilt(bf2, af2, rawmat(ch,:)); % band-pass filtered signal (for replacing low-freq oscillations after artifact removal)
...
%         tArtEnd = find(abs(chFilt(tPeak:end))>=abs(rawmat(ch,tPeak:end)),1,'first') + tPeak-1;
        ...
        tTrans = tArtEnd + find((artremov(tArtEnd:end-1)>chFilt2(tArtEnd:end-1)+rawmat(ch,end))&(artremov(tArtEnd+1:end)<chFilt2(tArtEnd+1:end)+rawmat(ch,end)),1,'first'); % find timepoint at which to start adding chFilt2
        sig_artremov(ch,:) = artremov;
        if isempty(tTrans)  % if chFilt2+rawmat(ch,end) doesn't overlap with artremov
            [~,tTrans_alt] = min(abs(chFilt2(tArtEnd+100:end)+rawmat(ch,end)-artremov(tArtEnd+100:end))); % point at which chFilt2+rawmat(ch,end) is closest to artremov (arbitrarily start looking from 5ms after tArtEnd)
...
%             sig_artremov(ch,tArtEnd:end) = artremov(tArtEnd:end)+linspace(0,rawmat(ch,end)-artremov(tArtEnd-1),length(chFilt(tArtEnd:end)));
            ...
            sig_artremov(ch,tTrans:end) = artremov(tTrans:end)+rawmat(ch,end)-artremov(end)+chFilt2(tTrans:end); % rawmat(ch,end)-artremov(end) so that rawmat(ch,end)==sig_artremov(ch,end)
        end
    end
...
for i=1:length(lightOFF_all)
    ...
    for ch = 1:nchs
        ...
        baseline = mean(rawmat(ch,1:50));
        tStart = find(abs(rawmat(ch,:)-baseline)>=5*std(rawmat(ch,1:50)),1,'first'); % first timepoint >5stds beyond baseline
        [~,tPeak] = max(rawmat(ch,tStart:tStart+20));  % maximum within 20ms of tStart (changed from abs(max) to max to be exclusive to light offsets )
        tPeak = tPeak + tStart-1;
        if isempty(tPeak)
...
%         tArtEnd = find(abs(chFilt(tPeak:end))>=abs(rawmat(ch,tPeak:end)),1,'first') + tPeak-1;
        ...
        if isempty(tTrans)
            ...
            ramp_st = find(artremov(tArtEnd:end)-baseline<2*std(artremov(tArtEnd:end)),1,'first')+tArtEnd-1; % point at which to start the ramp (first point <2stds away from baseline)
            sig_artremov(ch,ramp_st:tTrans_alt-1) = artremov(ramp_st:tTrans_alt-1)+linspace(0,chFilt2(tTrans_alt)+rawmat(ch,end)-baseline,length(ramp_st:tTrans_alt-1)); % make linear rampup to chFilt2 level
            sig_artremov(ch,tTrans_alt:end) = artremov(tTrans_alt:end)+rawmat(ch,end)-artremov(end)+chFilt2(tTrans_alt:end); % and then from there, add in chFilt2 (rawmat(ch,end)-artremov(end) so that rawmat(ch,end)==sig_artremov(ch,end))
%             sig_artremov(ch,tArtEnd:end) = artremov(tArtEnd:end)+linspace(0,rawmat(ch,end)-artremov(tArtEnd-1),length(chFilt(tArtEnd:end)));
        else
            sig_artremov(ch,tTrans:end) = artremov(tTrans:end)+rawmat(ch,end)-artremov(end)+chFilt2(tTrans:end); % rawmat(ch,end)-artremov(end) so that rawmat(ch,end)==sig_artremov(ch,end)
        end
